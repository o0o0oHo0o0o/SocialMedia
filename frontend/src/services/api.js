...
const api = {
  ...
  refresh: async () => {
    ...
    try {
      console.log("[API] POST", url, "with credentials");
    } catch (e) {}
    ...
    try {
      console.log("[API] refresh status", resp.status);
    } catch (e) {}
    return resp.ok;
  },
  // Conversations list
  getConversations: async () => {
    ...
    if (response.status === 401) {
      ...
      if (ok) {
        ...
        if (meData) {
          try {
            console.log("[API] Auth verified, retrying GET", url);
          } catch (e) {}
    ...
    try {
      console.log(
        "[API] conversations response status",
        response.status,
        "OK=",
        response.ok,
      );
    ...
    if (!response.ok)
      throw new Error(`Fetch conversations failed (HTTP ${response.status})`);
  ...
  getMessages: async (conversationId, limit = 50, page = 1) => {
    ...
    try {
      console.log("[API] GET", url, "with credentials");
    } catch (e) {}
    let response = await fetch(url, {
      method: "GET",
      credentials: "include",
    ...
    try {
      console.log("[API] messages status", response.status);
    } catch (e) {}
    if (!response.ok)
      throw new Error(`Fetch messages failed (HTTP ${response.status})`);
    return response.json();
  },
  me: async () => {
    try {
      ...
      if (response.status === 401) {
        ...
        if (ok) {
          try {
            console.log("[API] Retrying GET", url);
          } catch (e) {}
          response = await fetch(url, {
            method: "GET",
            credentials: "include",
  ...
  register: async (data) => {
    try {
      ...
      if (err?.message === "Failed to fetch") {
        throw new Error(
          "Không thể kết nối máy chủ. Kiểm tra API_BASE_URL, CORS và server backend.",
        );
      }
      throw err;
    }
  ...
  login: async (data) => {
    try {
      ...
      if (!response.ok) {
        ...
        const err = new Error(`${message} (HTTP ${response.status})`);
        try {
          console.error("[API /auth/login] Error:", response.status, message);
        } catch (e3) {
          void e3;
        }
        throw err;
  ...
  resendOTP: async (data) => {
    try {
      ...
      if (!response.ok) {
        ...
        try {
          console.error(
            "[API /verification/resend-otp] Error:",
            response.status,
            message,
          );
        } catch (e3) {
  ...
  sendChatMessage: async ({ jsonData, files }) => {
    ...
    const getXsrfToken = () =>
      getCookie("XSRF-TOKEN") || getCookie("X-XSRF-TOKEN");
    ...
    if (response.status === 401) {
      ...
      if (ok) {
        ...
        if (meData) {
          console.log(
            "[SendMessage] Got me data, rebuilding FormData for retry",
          );
          ...
          if (accessTokenRetry)
            retryHdrs["Authorization"] = `Bearer ${accessTokenRetry}`;
          const retryHeadersFinal = Object.keys(retryHdrs).length
            ? retryHdrs
            : undefined;
  ...
  getWebSocketToken: async () => {
    ...
    if (response.status === 401) {
      ...
      if (ok) {
        ...
        const meData = await api.me();
        if (meData) {
          response = await fetch(`${API_BASE}/chat/ws-token`, {
            method: "POST",
            credentials: "include",
          });
        }
    ...
    if (!response.ok)
      throw new Error(`WS token failed (HTTP ${response.status})`);
    return response.json();
  },
  ...
  reactToMessage: async ({ messageId, reactionType }) => {
    ...
    const url = `${API_BASE}/chat/reactions`;
  ...
  getMessageReactions: async (messageId) => {
    const url = `${API_BASE}/chat/messages/${messageId}/reactions`;
