...
import { ReactionApi } from "../../utils/ultis";
const Reaction = ({ userId, interactableId, entityId, targetType }) => {
  ...
  useEffect(() => {
    ReactionApi.fetchReaction(interactableId)
      .then((data) => {
        setReactions(data);
      })
      .catch((err) => {
            ...
            try {
              ...
              if (response.ok) {
                ...
                setReactions((prevReactions) => {
                  ...
                  const idx = updated.findIndex((r) => r.reactionType === name);
                  if (action === 'ADDED') {
                    if (idx !== -1) {
                      updated[idx].reactionCount = (updated[idx].reactionCount || 0) + 1;
                      updated[idx].hasUserReaction = true;
                    } else {
                      updated.push({ reactionType: name, reactionCount: 1, hasUserReaction: true });
                    }
                  } else if (action === 'REMOVED') {
                    if (idx !== -1) {
                      updated[idx].reactionCount = Math.max(0, (updated[idx].reactionCount || 1) - 1);
                      updated[idx].hasUserReaction = false;
                    }
                  } else if (action === 'UPDATED') {
                    // For simplicity, just refetch for now
                    ReactionApi.fetchReaction(interactableId).then(setReactions);
                  }
                  return updated;
                });
              }
